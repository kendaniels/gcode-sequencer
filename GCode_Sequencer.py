...
parser = argparse.ArgumentParser(description='Name the output file.')
parser.add_argument('project_name', help='provide a name for the project')
...
def run(options: Dict[str, Union[int, float, List[str]]], file_list: List[str], save_file: TextIO, pre_gcode: str, mid_gcode: str, post_gcode: str, output_file_counter=1):
	...
	print("Number of files to sequence: " + str(len(file_list)))
	...
	while file_counter < len(file_list):
		...
		print("Working with file # " + str(file_counter + 1) + ": " + file_list[file_counter])
		...
		while options['bed_y_min'] <= coord['y'] <= options['bed_y_max'] and options['bed_y_min'] <= coord['y'] + model_y_dim <= options['bed_y_max'] and not is_placed:
			while options['bed_x_min'] <= coord['x'] <= options['bed_x_max'] and options['bed_x_min'] <= coord['x'] + model_x_dim * sign['x'] <= options['bed_x_max'] and not is_placed:
				...
				if is_placed:
					...
					# move placed files into the projects /placed/ directory
					print("Model placed")
					file_name = ntpath.basename(file_list[file_counter])
					shutil.move(file_list[file_counter], "projects/" + project_folder + "/placed/" + file_name)
	...
	files_left = glob.glob("gcode/*.gcode")
	if len(files_left) > 0:
		print("but we still have models not placed...create a new file and add those")
		output_file_counter += 1

		print("creating file number " + str(output_file_counter))

		_save_file = open('projects/' + project_name + "/" + project_name + "-" + str(output_file_counter) + ".gcode" , 'w')

		run(_options, files_left, _save_file, _pre_gcode, _mid_gcode, _post_gcode, output_file_counter)
	else:
...
if __name__ == '__main__':
	...
	_file_list = glob.glob("gcode/*.gcode")
	...
	with open('post.gcode') as _f:
		_post_gcode = _f.read()
	with open('projects/' + project_name + "/" + project_name + "-" + str(output_file_counter) + ".gcode" , 'w') as _save_file:
